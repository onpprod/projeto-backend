...
async def listar_produtos():
    ...
    if len(lista_supermercado) > 0 :
        ...
        message = {"message": "Carrinho vazio!"}
...
async def listar_produtos_comida():
    ...
    nova_lista = [dicionario for dicionario in lista_supermercado if dicionario.get("tipo") == "comida"]

    if len(nova_lista) > 0 :
        message = nova_lista
    else:
        message = {"message": "Nenhum item de comida encontrado!"}
...
async def listar_produtos_bebida():
    ...
    nova_lista = [dicionario for dicionario in lista_supermercado if dicionario.get("tipo") == "bebida"]

    if len(nova_lista) > 0:
        message = nova_lista
    else:
        message = {"message": "Nenhum item de bebida encontrado!"}
...
async def atualizar_produto(produto: Produto):
    ...
    for produto_a_atualizar in lista_supermercado:
        if produto_a_atualizar['item'] == pd["item"]:
            lista_supermercado.remove(produto_a_atualizar)
            found = True
            break

    if not found:
        message = {"message": "Produto " + pd["item"] + " não foi encontrado na lista!"}
    else:
        lista_supermercado.append(produto.dict())
        message = {"message": "Produto " + pd["item"] + " atualizado com sucesso na lista!"}
...
async def apagar_lista_item(item:Produto):
    """Método que será chamado quando for requisitada a rota DELETE /produtos/"""

    global lista_supermercado
    if len(lista_supermercado)>0:
        nova_lista = [dicionario for dicionario in lista_supermercado if dicionario.get("item") != item.dict()['item']]
        lista_supermercado = nova_lista
        message = {"message": "Item apagado com sucesso!"}
    else:
        message = {"message": "Lista vazia!"}
